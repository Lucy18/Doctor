...
public class BaseDAO {
    ...
    public <T> T select(Class<T> c, Object id, String[] attrs) {
        String tname = c.getAnnotation(Table.class).name();
        return j.queryForBean(c, "select id," + DataUtil.array2String(attrs) + " from " + tname + " where id=?", new Object[]{id});
    }
            ...
            for (String arg : args) {
                ...
                if (argArr.length == 2) {
                    if ("time".equals(argArr[1]))
                        subMap.put(argArr[0], sdf.format(DataUtil.getLong(map.get(argArr[0])) * 1000));
                    else
